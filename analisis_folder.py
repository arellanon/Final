...
def main():
    with open('result.csv', 'w', newline='') as csvfile:
        fieldnames = ['file', 'accuracy']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)    
        writer.writeheader()
        
    #file = "D4SBI_S02_FI4S8ACH"
    folder_input = "Epoch/DATA3/"
    
    for root, dirs, files in os.walk(folder_input):
        for file in files:
            filename=os.path.join(root, file)    
            trials1, info1 = get_epoch(filename)
            accuracy = calculateML(trials1, info1, False)
            print('Accuracy: %.3f' %  accuracy)
            with open('result.csv', 'a', newline='') as csvfile:
                fieldnames = ['file', 'accuracy']
                writer = csv.DictWriter(csvfile, fieldnames=fieldnames)    
                writer.writerow({'file': file, 'accuracy': accuracy})
